...
def score_test(results):
    ...
    log(f"  PASS ✓ — Dissociation confirmed with delta-free Ω")
    ...
    log(f"\n--- Comparison (a): C(4-40) vs Ω(4-40) ---")
    ca = boot_dd(pre_d["C_df"],stb_d["C_df"],pre_d["omega_df"],stb_d["omega_df"],
                 LOCKED["n_bootstrap"],LOCKED["bootstrap_seed"])
    log(f"  Δd={ca['delta_d']:.3f} CI=[{ca['ci_low']:.3f},{ca['ci_high']:.3f}] p={ca['p_value']:.4f} pass={ca['passes']}")

    # Comparison (b): C_df vs kappa
    log(f"\n--- Comparison (b): C(4-40) vs κ ---")
    cb = boot_dd(pre_d["C_df"],stb_d["C_df"],pre_d["kappa"],stb_d["kappa"],
                 LOCKED["n_bootstrap"],LOCKED["bootstrap_seed"])
    log(f"  Δd={cb['delta_d']:.3f} CI=[{cb['ci_low']:.3f},{cb['ci_high']:.3f}] p={cb['p_value']:.4f} pass={cb['passes']}")

    # Comparison (c): C_mult vs C_add
    log(f"\n--- Comparison (c): C_mult vs C_add ---")
    cc = boot_dd(pre_d["C_df"],stb_d["C_df"],pre_d["C_add"],stb_d["C_add"],
                 LOCKED["n_bootstrap"],LOCKED["bootstrap_seed"])
    log(f"  Δd={cc['delta_d']:.3f} CI=[{cc['ci_low']:.3f},{cc['ci_high']:.3f}] p={cc['p_value']:.4f} pass={cc['passes']}")
    ...
    if a_pass and b_pass:
        if ca["delta_d"] >= 0.5: score = "C3"; log(f"  C beats both, Δd≥0.5 → C3")
...
def main():
    ...
    log("Pre-registered: 2026-02-08 | Stop rule: corr(Ω_4-40, κ)")
    log("="*60)
